...
test.describe("目次 (TOC)", () => {
  test("長文ページに sidebar / mobile 両 variant の TOC が描画される", async ({
    page,
  }) => {
    await page.goto("/strategies/feedback/");
    await expect(
      page.locator('[data-toc][data-toc-variant="sidebar"]')
    ).toHaveCount(1);
    await expect(
      page.locator('[data-toc][data-toc-variant="mobile"]')
    ).toHaveCount(1);
  ...
  test("スクロールに応じて aria-current が現在セクションに追従する", async ({
    page,
  }) => {
    ...
    await page.waitForFunction(() => {
      return (
        document.querySelectorAll('[data-toc-link][aria-current="location"]')
          .length > 0
      );
    });
  ...
  test("モバイル TOC details の開閉で data-state が連動する", async ({
    page,
  }) => {
