...
1 エントリの文体と粒度は [ADR 0023](docs/decisions/0023-unify-changelog-register-and-granularity.md) が正典。
...
- **執筆系のフロー**(`pre-write-verifier` → 執筆 → `content-reviewer` →(数値・帰属を含むもののみ)`adversarial-verifier`)は、新規コラム / 戦略ページを書くたびに回す
