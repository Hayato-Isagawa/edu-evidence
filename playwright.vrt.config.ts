...
export default defineConfig({
  ...
  // 立たず、どちらも `colorScheme: "dark"` の project がそのまま light を描く(2026-09-14
  // 時点で実測)。`use` の中身は `scripts/__tests__/workflows/vrt-targets.test.mjs` が
  // 丸ごと固定している。
  //
  // ダークを撮るまで **1 枚も写っていなかった**。ダーク側は `[data-theme="dark"]` で
  // 色トークンを 15 宣言まとめて差し替え、テンプレートの `dark:` 変種が 7 か所ある形
  // なので、**全ページの見た目が変わるのにピクセル検査は 1 度も通っていない**状態だった。
  // 色のコントラストは `e2e/a11y.spec.ts` が両テーマで見ているが、そちらはレイアウトの
  // 崩れを見ない(a11y が拾えて VRT が拾えない側)。
  projects: [
    {
      name: "desktop",
