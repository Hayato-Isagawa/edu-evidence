...
- `npm run check:text` / `npm run build` / `npm run test:e2e` の実行結果
- 本番計測などの観測値
...
- **2026-05-14 初回整理**: 3,372 行 → 667 行(直近 3 日分 + 進行中 Dependabot 自動マージ整備プロジェクトを残す)。SessionStart hook で 1,500 行警告を自動感知するよう同時整備。
- **2026-05-23 2 回目整理**: 1,531 行 → 211 行(セッション #30 の archive 整理。直近 1〜2 セッション分 = セッション #28-#29 連続作業 + #30 を残す)。SessionStart hook 1,500 行警告を契機に実施。

### 例外と運用の幅

- 6 サブ節すべてが毎回必須ではない。短いセッションでは「本タスクで作成 / 変更」+「実装ノート」+「Next Action」だけでもよい
- 「検証」だけが目的のセッション(数値計測のみ等)では「本タスクで作成 / 変更」を「観測した数値」に読み替えて使う
- 姉妹サイト固有の運用ポイント(本リポジトリでは下記の節)は別見出しで補足してよい
