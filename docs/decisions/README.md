...
- **原則として改変しない**(誤植修正は例外)
- **ADR には `memory rule N` / `ルール N` という、リポジトリ外の個人メモを指す番号参照が残っている。** 番号そのものは第三者が辿れない。「誤植修正は例外」に言い換えは含まれないので、既存の ADR に残った番号はそのままにする(同じ行か同じファイルの参照リストに規範の内容が併記してあり、番号を辿れなくても趣旨は読める)。**新規 ADR では番号で参照しない** — 規範の内容を本文に書き切るか、リポジトリ内の文書を指す
