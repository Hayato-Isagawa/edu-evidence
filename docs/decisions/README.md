...
- **原則として改変しない**(誤植修正は例外)
- **既存 ADR に残る `memory rule N` / `rule N` は、リポジトリ外の個人メモを指す番号なので第三者は辿れない。** 言い換えは「誤植修正」にあたらないのでそのままにする(多くは同じ行か参照リストに趣旨が併記されている)
- **新規 ADR では番号で参照しない** — 規範の内容を本文に書き切るか、リポジトリ内の文書を指す
