...
直近 60 日にマージされた Dependabot PR 41 件は、すべて VRT の対象外だった（2026-09-11 実測）。

ADR 0027 の検証は手動の移行 PR で、ソースも触っていたから走ったにすぎない。「見るべきもの」は意図で
あって実績ではなかった。
...
- **非 major の bump ではゲートにならない。** required check は Content Checks と E2E（合わせて
  1.5 分程度）で、`dependabot-auto-merge.yml` の `gh pr merge --auto` はそれしか待たない。
  Dependabot PR は作成から約 2 分でマージされる（#566 で実測）のに対し VRT は 5 分前後なので、
  結果はマージ後に出る。残るのは PR 上の check の色と、7 日で消える差分画像（artifact の
  `retention-days: 7`）だけ
...
| 「`src/data/**` を `paths` に残すと…15 分の CI を空回りさせる」 | 1 run は 5 分前後。15 分は直近 30 run のどれとも一致せず、再現できなかった（timeout は 45 分） |
