...
- **戦略個別ページのような中核メタブロックを持つページ** は、H1 だけでなくページの主要情報の一部(効果量 / エビデンス強度 / コスト / 出典バッジなど)が「ページ階層上の主軸」であり、H1 直下に縦積みすると主軸情報の優先度が下がる。このようなページでは **12-col grid の 2 列レイアウト**(左: kicker + H1 + 説明文 + タグ、右: メタブロック)を維持してよい。H1 サイズは個別記事系の例外と同じ `text-3xl sm:text-4xl md:text-5xl lg:text-6xl` を使う
