...
- 教師の包括責任観から距離を取る。家庭・地域の責任境界を明示する(`docs/CONTENT_GUIDELINES.md` Rule 1.6)
...
- `monthsGained` — 効果量(月数換算、整数 -12〜12)。**一次研究で裏付けたものだけ書く**(`docs/CONTENT_GUIDELINES.md` Rule 1.1)
...
- `evidence.eef.kind` — 値の出所(`toolkit` / `early-years-toolkit` / `trial` / `evidence-review`、optional。省略時は `toolkit`)。戦略ページの EEF カードの見出しを切り替える
...
```text
[reward]       ご褒美 / 報酬 / 外発的動機付け / インセンティブ          <- PR #114 の見落とし(2026-04-26)由来
...
- 一次研究ドメイン限定: `docs/CONTENT_GUIDELINES.md` Rule 1.2b
- glossary 等メタデータ層の定義も一次源で裏を取る(`src/data/glossary.ts` の定義文と数値): 同 Rule 1.1
- コンテンツ編集の前にプランモードで対象ファイルと出典を宣言し、承認を得てから抜ける
  (`src/content/` 配下・changelog・ADR)
- changelog は読み手の行動や体験が変わる変更だけ書く: `CONTRIBUTING.md`「changelog を同じ PR で更新する」/ ADR 0023
- semantic 属性で UI 状態管理: ADR 0007
- 直線 3px アクセントバー範囲: ADR 0008
- TDD は vertical slicing で進める: 1 test → 1 実装 → 次の 1 test(全テストを先に書かない)
