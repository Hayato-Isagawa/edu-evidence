...
- `archive.ph` / Bing cache → 取得不可。Wayback Machine は `id_` の生バイト取得なら通る(上表の優先度 0。2026-09-17 に #612 / #613 の照合で 13 ページ実測)
