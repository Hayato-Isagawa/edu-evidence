...
test("ベースラインが PR の素材を --delete 付きで運んでいる", () => {
  ...
  assert.match(b, /^ +cp src\/content\.config\.ts \/tmp\/evi-main\/src\/content\.config\.ts$/m);
});
...
test("運ぶディレクトリは paths に載っていない", () => {
  ...
  // 15 分の CI を空回りさせる。対象 0 件で緑になる経路そのもの。
  const watched = paths();
  for (const dir of CARRIED) {
    assert.ok(
...
test("degraded 側の撮り直しは失敗を握りつぶさない", () => {
  ...
  assert.match(fallback, /npm --prefix \/tmp\/evi-main run build/, "撮り直しが無い");
...
test("degraded の復元が Astro のキャッシュまで消している", () => {
  ...
  for (const cache of ["/tmp/evi-main/.astro", "/tmp/evi-main/node_modules/.astro"]) {
