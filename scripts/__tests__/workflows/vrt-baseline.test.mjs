// VRT のベースラインを「main のコード x PR のコンテンツ」で撮る配線を固定する（ADR 0034）。
//
...
test("ベースラインが PR の素材を --delete 付きで運んでいる", () => {
  ...
  for (const dir of CARRIED) {
    assert.match(
      b,
      new RegExp(`^ +rsync -a --delete ${dir}/ /tmp/evi-main/${dir}/$`, "m"),
  ...
  assert.match(
    b,
    /^ +cp src\/content\.config\.ts \/tmp\/evi-main\/src\/content\.config\.ts$/m
  );
  // **ファイルと worktree を動かす行は、この列挙と完全一致させる。** 上の 3 つが在る
  // ことしか見ないと、`rsync -a --delete src/ /tmp/evi-main/src/` や
  // `git -C /tmp/evi-main checkout "$GITHUB_SHA" -- src` を 1 行足して描画コード
  ...
  assert.deepEqual(carries, [
    ...
    "mv /tmp/evi-main/dist dist-main", // raw(neutral=false)の早期 exit
    "rsync -a --delete src/content/ /tmp/evi-main/src/content/",
    "rsync -a --delete src/data/ /tmp/evi-main/src/data/",
    "cp src/content.config.ts /tmp/evi-main/src/content.config.ts",
    "git -C /tmp/evi-main checkout -- .",
    "git -C /tmp/evi-main clean -fd",
    "rm -rf /tmp/evi-main/dist /tmp/evi-main/.astro /tmp/evi-main/node_modules/.astro /tmp/evi-main/node_modules/.vite",
...
test("素材を運ぶのがベースラインのビルドより前である", () => {
  ...
  const build = afterRawBranch.indexOf("npm --prefix /tmp/evi-main run build");
...
test("運ぶディレクトリは paths に載っていない", () => {
  ...
  // 15 分近い CI を空回りさせる(100 枚で 14 分 31 秒。vrt.yml の paths コメント参照)。
  // 対象 0 件で緑になる経路そのもの。
  const watched = paths();
  for (const dir of CARRIED) {
    assert.ok(
...
test("degraded への分岐がビルドコマンドだけに掛かっている", () => {
  ...
  assert.deepEqual(tolerated, ["npm --prefix /tmp/evi-main run build"]);
});
...
test("degraded の復元が Astro のキャッシュまで消している", () => {
  ...
  for (const cache of [
    "/tmp/evi-main/.astro",
    "/tmp/evi-main/node_modules/.astro",
  ]) {
    assert.ok(fallback.includes(cache), `${cache} を消していない`);
  }
  assert.match(fallback, /git -C \/tmp\/evi-main checkout -- \./);
  assert.match(fallback, /git -C \/tmp\/evi-main clean -fd/);
});
