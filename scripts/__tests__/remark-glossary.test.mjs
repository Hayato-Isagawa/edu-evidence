...
function toHtml(nodes) {
  return nodes
    .map((n) => {
      ...
      if (n.type === "link")
        return `<a href="${n.url}">${toHtml(n.children)}</a>`;
...
function linkedTerms(html) {
  return [...html.matchAll(/class="glossary-tip"[^>]*>([^<]*)<\/a>/g)].map(
    (m) => m[1]
  );
}
...
test("長い用語に含まれる短い用語は、その中では別にリンクしない", () => {
  ...
  // 「RCT」は初出として拾う。
  const html = toHtml(
    render([text("日本初のクラスターRCTを公表。RCT 一般とは異なる。")])
  );
  ...
  assert.match(
    html,
    /クラスターRCT<\/a>を公表。<a class="glossary-tip"[^>]*>RCT<\/a> 一般/,
    html
  );
...
test("同じ用語は文書内で初出の 1 回しかリンクされない", () => {
  ...
  assert.match(
    html,
    /^<a class="glossary-tip"[^>]*>効果量<\/a>の話。あとで効果量が再び出る。/,
    html
  );
});
...
test("リンクの中とインラインコードの中の用語は触らない", () => {
  ...
  assert.match(
    html,
    /本文の<a class="glossary-tip"[^>]*>効果量<\/a>はここ。$/,
    `本文側の初出がリンクされていない:\n${html}`
  );
});
