...
evidence:
  japan:
    monthsGained: 2
    strength: 2
    note: "**特別活動(Tokkatsu)は日本独自の教育領域** で、学級活動・児童会・クラブ・学校行事を通じた自治・協働・社会性の育成を担う。学力への直接効果の大規模実証研究は限定的だが、**海外でも Tokkatsu の国際展開(エジプト・インド・ベトナム等)**が進み、JICA・国立教育政策研究所が事例集を蓄積。EEF の SEL(+3)・cooperative-learning(+5)と重なる要素を多く含む。"
...
効果量(+2ヶ月)は推定値であり、特別活動の効果を検証したRCTは存在しません。特別活動は日本固有の教育領域であるため、海外の比較研究も限られています。非認知能力への正の効果は実践研究で繰り返し報告されていますが、学力への直接的な因果効果は実証されていません。
