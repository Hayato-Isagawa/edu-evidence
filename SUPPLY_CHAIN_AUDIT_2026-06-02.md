...
□ Playwright を Chromium 149+ 同梱版へ更新 ― ただし Chromium 149 安定版を同梱する Playwright 1.61.0 安定版は未リリース（2026-06-03 時点で alpha のみ。1.60.0 同梱の 149 は tip-of-tree 開発版で安定版ではない）。Dependabot（npm, weekly 設定済）が 1.61.0 安定版公開後に自動で bump PR を作成 → CI 通過後 auto-merge。手動での alpha / tip-of-tree 導入は本番 E2E の安定性を損なうため行わない
...
次のアクション:
1. （低）Playwright 1.61.0 安定版（Chromium 149 同梱）の公開待ち。Dependabot が自動で bump PR を作成し、CI 通過後 auto-merge されるため手動更新は不要（alpha / tip-of-tree は使わない）
