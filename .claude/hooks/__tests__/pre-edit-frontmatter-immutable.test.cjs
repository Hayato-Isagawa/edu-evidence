...
const manyUrls = (tag, count) =>
  Array.from({ length: count }, (_, i) => `  url: https://doi.org/10.${tag}/edu.${i}`).join('\n');

test('CLI: 判定が 64KB を超えても stdout が切れない', () => {
  const res = runCli(editOn(STRATEGY, manyUrls('1111', 1200), manyUrls('2222', 1200)));
...
test('CLI: 判定の理由は stderr にも出る', () => {
  ...
  assert.match(res.stderr, /sourceUrl is primary research only\) before applying\.\n$/);
});
...
test('Write: sourceUrl の差し替えも検知する', () => {
  const p = strategyFile(BODY);
  const out = writeOn(p, BODY.replace('https://example.org/a', 'https://example.org/b'));
  assert.ok(fired(out));
});

test('Write: 保護フィールドが同じなら素通りする', () => {
  const p = strategyFile(BODY);
  assert.equal(fired(writeOn(p, BODY.replace('本文', '本文を推敲した'))), false);
});
