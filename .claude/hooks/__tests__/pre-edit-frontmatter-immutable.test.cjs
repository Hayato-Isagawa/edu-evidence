...
test("TARGET_PATH_RE: matches strategies/columns paths", () => {
  ...
  assert.match(
    "/Users/H/edu-evidence/src/content/strategies/z.md",
    TARGET_PATH_RE
  );
});
...
test("フェンス無しの Edit チャンクでも保護フィールドの変更を検知する", () => {
  assert.equal(
    fired(run(editOn(STRATEGY, "monthsGained: 5", "monthsGained: 9"))),
    true
  );
});
...
test('リスト項目の形（"- key: v"）でも検知する', () => {
  assert.equal(
    fired(run(editOn(STRATEGY, "  - year: 2019", "  - year: 2021"))),
    true
  );
});
...
test("本文だけの編集は誤検知しない", () => {
  const out = run(
    editOn(STRATEGY, "この指導法は有効です。", "この指導法は有効でした。")
  );
...
test("対象外パスならフェンス無しでも素通りする", () => {
  const out = run(
    editOn("src/lib/util.ts", "monthsGained: 5", "monthsGained: 9")
  );
  assert.equal(fired(out), false);
});
...
// 戻っていないかを見る。
//
// **サイズと閾値は実測で決めた**(本機 Node 24、保護キー 12 本):
//   32KB / 線形       1.6ms
//   32KB / 全キー二次  20,376ms
//   32KB / 1 キーだけ二次  1,696ms   ← 部分的な退行もこの閾値で捕まる
// 16KB では 1 キーだけの退行が 423ms で閾値を割らない。逆に 256KB まで広げると
// 検知力は変わらないまま、赤になるまでに 20 分かかる。
...
// url を多数持つ frontmatter の全文書き換えでこの大きさに届く。

const manyUrls = (tag, count) =>
  Array.from(
    { length: count },
    (_, i) => `  url: https://doi.org/10.${tag}/edu.${i}`
  ).join("\n");

test("CLI: 判定が 64KB を超えても stdout が切れない", () => {
  const res = runCli(
    editOn(STRATEGY, manyUrls("1111", 1200), manyUrls("2222", 1200))
  );
  ...
  assert.match(
    parsed.hookSpecificOutput.permissionDecisionReason,
    /sourceUrl is primary research only\) before applying\.$/
  );
...
test("CLI: 判定の理由は stderr にも出る", () => {
  const res = runCli(editOn(STRATEGY, "monthsGained: 5", "monthsGained: 9"));
  assert.match(res.stderr, /monthsGained/);
  assert.match(
    res.stderr,
    /sourceUrl is primary research only\) before applying\.\n$/
  );
...
test("Write: 保護フィールドが変わったら確認を出す", () => {
  const p = strategyFile(BODY);
  const out = writeOn(p, BODY.replace("monthsGained: 5", "monthsGained: 9"));
  assert.ok(fired(out), "Write による全文書き換えを検知できていない");
  assert.match(out.stdout, /monthsGained/);
});

test("Write: sourceUrl の差し替えも検知する", () => {
  const p = strategyFile(BODY);
  const out = writeOn(
    p,
    BODY.replace("https://example.org/a", "https://example.org/b")
  );
  assert.ok(fired(out));
});
...
test("Write: 保護フィールドが同じなら素通りする", () => {
  const p = strategyFile(BODY);
  assert.equal(
    fired(writeOn(p, BODY.replace("本文", "本文を推敲した"))),
    false
  );
});

test("Write: 新規作成(ENOENT)は比較対象が無いので素通りする", () => {
  const p = strategyFile(undefined); // ファイルを作らない
  assert.equal(fired(writeOn(p, BODY)), false);
});
...
test("Write: 現物を読めないときは素通りさせず確認を出す(fail-safe)", () => {
  ...
  const out = writeOn(p, BODY);
  assert.ok(fired(out), "読めないのに素通りしている");
