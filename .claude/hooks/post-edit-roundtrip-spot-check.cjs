...
function buildContext(diffs, filePath) {
  ...
  lines.push("");
  lines.push(
    "Before continuing, briefly state which primary research source supports"
  );
  lines.push(
    "each numeric or URL change. If unsure, ask the user before further edits."
  );
  return lines.join("\n");
}
...
if (require.main === module) {
  ...
  process.stdin.on("end", () => {
    ...
    // 直後に exit すると書き残しが捨てられ、JSON がちょうど 65536B(パイプバッファ)
    // で切れる。切れた JSON は誰もエラーにせず、global ディスパッチャは
    // 「判定ではない付随出力」として捨てて exit 0 = **nudge が無音で消える**。
    // exitCode を置くだけにして、Node に flush させる。
